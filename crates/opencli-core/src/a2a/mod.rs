...
pub async fn run_subagent(
    ...
    if delegation_depth >= config.a2a_max_depth {
        bail!(
            "a2a delegation depth exceeded limit {}",
            config.a2a_max_depth
        );
    }
        ...
        if let Some((index, request)) = iter.next() {
            ...
            futures.push(
                async move {
                    (
                        index,
                        run_subagent(config, request, cancel_requested, delegation_depth, parent)
                            .await,
                    )
                }
                .boxed_local(),
...
mod tests {
    ...
    async fn run_subagents_concurrent_preserves_request_order() {
        ...
        .await
        .expect("concurrent subagent run should succeed");
    ...
    async fn run_subagents_concurrent_returns_partial_failures() {
        ...
        .await
        .expect("concurrent subagent run should return partial failures");
