...
#[test]
fn markdown_fixture_renders_as_expected() {
    let input = fs::read_to_string(fixture_path("markdown_sample.md"))
        .expect("markdown input fixture should be readable");
    let expected = fs::read_to_string(fixture_path("markdown_sample.txt"))
        .expect("markdown expected fixture should be readable");
    let rendered = opencli_markdown_render_for_test(&input);
...
fn opencli_markdown_render_for_test(input: &str) -> String {
    ...
    for event in Parser::new_ext(input, Options::all()) {
        match event {
            ...
            Event::Text(text) => {
                if in_blockquote && (out.is_empty() || out.ends_with('\n')) {
                    out.push_str("> ");
                }
