...
impl Default for RuntimeConfig {
    fn default() -> Self {
        Self {
            ...
            session_dir: "~/.config/ai-cli/sessions".to_string(),
            audit_log_path: "~/.config/ai-cli/audit.jsonl".to_string(),
...
pub fn load_config(overrides: ConfigOverrides) -> Result<RuntimeConfig> {
    ...
    if config.audit_log_path.is_empty() {
        config.audit_log_path = "~/.config/ai-cli/audit.jsonl".to_string();
    }
    ...
    if let Some(model) = overrides.model.or_else(|| env::var("AI_CLI_MODEL").ok()) {
        config.model = model;
    }
    if let Some(base_url) = overrides
        .base_url
        .or_else(|| env::var("AI_CLI_BASE_URL").ok())
    {
        config.base_url = base_url;
    }
    if let Some(api_key) = overrides
        .api_key
        .or_else(|| env::var("AI_CLI_API_KEY").ok())
    {
