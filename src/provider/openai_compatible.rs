...
use super::{ChatResponse, Provider, ProviderCapabilities, util::{build_client, ensure_success, serialize_openai_message, serialize_openai_tool}};
...
impl Provider for OpenAiCompatibleProvider {
    ...
    async fn complete_chat(&self, messages: &[ChatMessage], tools: &[ToolDefinition], cancel_requested: Option<&Arc<AtomicBool>>) -> Result<ChatResponse> {
        ...
        let response = cancelable_request(
            self.client.post(format!("{}/chat/completions", self.base_url.trim_end_matches('/')))
        ...
        ensure_success(response).await?
            ...
            .map(|body| {
                let message = body.choices.into_iter().next().unwrap_or(OpenAiChoice {
                    message: OpenAiAssistantMessage { content: Some(String::new()), tool_calls: Vec::new() },
                }).message;

                ChatResponse {
                    content: message.content.unwrap_or_default(),
                    tool_calls: message.tool_calls.into_iter().map(|call| ToolCall {
                        id: call.id,
    ...
    async fn stream_text(&self, messages: &[ChatMessage], renderer: &mut dyn Renderer, cancel_requested: Option<&Arc<AtomicBool>>) -> Result<String> {
        ...
        while let Some(chunk) = cancelable_response_chunk(&mut response, cancel_requested).await? {
            ...
            while let Some(index) = buffer.find('\n') {
                ...
                if let Some(text) = parsed.pointer("/choices/0/delta/content").and_then(Value::as_str) {
                    debug!(provider = "openai-compatible", chunk_len = text.len(), "received text delta");
