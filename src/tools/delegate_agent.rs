...
impl Tool for DelegateAgentTool {
    ...
    async fn execute(&self, args: Value, context: &ToolExecutionContext<'_>) -> Result<String> {
        let role = args.get("role").and_then(Value::as_str).context("delegate_agent.role is required")?;
        let task = args.get("task").and_then(Value::as_str).context("delegate_agent.task is required")?;
        let request = SubAgentRequest {
            role: role.to_string(),
            task: task.to_string(),
            context: args.get("context").and_then(Value::as_str).map(ToString::to_string),
            allowed_tool_kinds: args.get("allowed_tool_kinds").and_then(Value::as_array).map(|items| {
                items.iter().filter_map(Value::as_str).map(ToString::to_string).collect()
            }),
            allowed_tools: args.get("allowed_tools").and_then(Value::as_array).map(|items| {
                items.iter().filter_map(Value::as_str).map(ToString::to_string).collect()
            }),
            max_steps: args.get("max_steps").and_then(Value::as_u64).map(|v| v as usize),
        };
