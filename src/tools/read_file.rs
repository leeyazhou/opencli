...
impl Tool for ReadFileTool {
    ...
    async fn execute(&self, args: Value, context: &ToolExecutionContext<'_>) -> Result<String> {
        let path = args.get("path").and_then(Value::as_str).context("read_file.path is required")?;
        let file_path = resolve_workspace_path(&context.config.workspace_root, path)?;
