...
use std::{process::{Command, Stdio}, sync::atomic::Ordering, time::Duration};
...
impl Tool for RunShellTool {
    ...
    async fn execute(&self, args: Value, context: &ToolExecutionContext<'_>) -> Result<String> {
        let command = args.get("command").and_then(Value::as_str).context("run_shell.command is required")?;
        let workdir = args.get("workdir").and_then(Value::as_str).unwrap_or(".");
        context.approval.approve_shell(command, context.config)?;
        let cwd = resolve_workspace_path(&context.config.workspace_root, workdir)?;

        let output = Command::new(std::env::var("SHELL").unwrap_or_else(|_| "/bin/zsh".to_string()))
            .arg("-lc")
            .arg(command)
            .current_dir(cwd)
            .stdout(Stdio::piped())
        ...
        let output = wait_for_output(output, context.config.shell_timeout_ms, context.cancel_requested)?;
