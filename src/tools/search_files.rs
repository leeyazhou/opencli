...
impl Tool for SearchFilesTool {
    ...
    async fn execute(&self, args: Value, context: &ToolExecutionContext<'_>) -> Result<String> {
        let query = args.get("query").and_then(Value::as_str).context("search_files.query is required")?;
        let path = args.get("path").and_then(Value::as_str).unwrap_or(".");
        let include = args.get("include").and_then(Value::as_str);
        let search_path = resolve_workspace_path(&context.config.workspace_root, path)?;
    ...
    text.lines()
        .filter(|line| {
            ...
            pattern.matches(path)
                || path.rsplit('/').next().is_some_and(|name| pattern.matches(name))
        })
        .collect::<Vec<_>>()
        .join("\n")
}
