...
pub async fn run_chat_tui(app: &mut App) -> Result<()> {
    ...
    let result = loop {
        ...
        if state.pending.is_none() {
            if let Some(prompt) = state.queued_prompts.pop_front() {
        ...
        if let Event::Key(key) = event::read()? {
            ...
            match key.code {
                    ...
                    if state.pending.is_some() {
                        if state.cancel_pending_task {
                            if let Some(pending) = &state.pending {
                                pending.cancel_requested.store(true, std::sync::atomic::Ordering::Relaxed);
                            }
                            state.cancel_pending_task = false;
                            state.lines.push("Cancel requested for current task.".to_string());
                            state.scroll_from_bottom = 0;
                        } else {
                ...
                KeyCode::Up => state.scroll_from_bottom = state.scroll_from_bottom.saturating_add(1),
                KeyCode::Down => state.scroll_from_bottom = state.scroll_from_bottom.saturating_sub(1),
