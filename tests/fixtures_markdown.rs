...
fn markdown_fixture_renders_as_expected() {
    let input = fs::read_to_string("tests/fixtures/markdown_sample.md").unwrap();
    let expected = fs::read_to_string("tests/fixtures/markdown_sample.txt").unwrap();
    let rendered = ai_cli_markdown_render_for_test(&input);
    assert_eq!(rendered.trim(), expected.trim());
}

fn ai_cli_markdown_render_for_test(input: &str) -> String {
